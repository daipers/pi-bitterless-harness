...
- `Yellow` Runtime enforcement model: opt-in `managed_rpc` runs now live-intercept every subagent action and fail closed on denied actions, while legacy `rpc` runs still rely on pre-score audit plus score-time validation. See [starter/bin/run_task.py](./starter/bin/run_task.py), [starter/bin/score_run.py](./starter/bin/score_run.py), and [tests/test_runner_e2e.py](./tests/test_runner_e2e.py).
